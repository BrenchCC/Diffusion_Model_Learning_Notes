...
2. **利用提示（时间步 t）**：模型通过$t$算出了$w$ 。它知道："现在这张图里，有 40% 是我不想要的随机像素，有 60% 是我要保留的结构。"
3. **模式识别**：
* 模型看这张图，发现有些像素杂乱无章（符合高斯噪声分布），它判定这是**噪声**。
* 有些像素连成了线条（符合它记忆中的图像轮廓），它判定这是**信号**。
* 它尝试构造一张图，这张图在乘以$(1-w)$后，非常接近输入图中"有规律"的那一部分。
...
* **原图数值 ($x_{start}$)**: `100` (比如是天空的蓝色)
* **噪声数值 ($x_{noise}$)**: `10` (随机生成的)
* **时间步 ($t$)**: 对应权重 $w=5$
